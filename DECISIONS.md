...
El "Desafío de arquitectura para IA" ya está resuelto con esto en mente, y de hecho ya está conectado un proveedor externo real (OpenAI) para demostrarlo, no solo descrito en teoría:

1. `App\Services\ExecutiveSummary\OpenAiExecutiveSummaryService implements ExecutiveSummaryServiceInterface` arma un prompt con los mismos datos que ya calcula `DashboardMetricsService` (no duplica consultas), le pide a la Chat Completions API una respuesta JSON con `response_format: json_schema` (headline/body/highlights) y la mapea al mismo DTO `ExecutiveSummaryResult` que usa el motor local. El prompt y el schema de salida están en un trait compartido (`Concerns\BuildsExecutiveSummaryPrompt`) para no duplicar esa lógica si se agrega otro proveedor.
2. La API key vive en `.env` (`OPENAI_API_KEY`, `OPENAI_ORGANIZATION`, `OPENAI_MODEL`) → `config/services.php` → constructor, nunca hardcodeada. Usa el cliente HTTP de Laravel (`Http::withToken()->timeout()->retry()`) y convierte cualquier fallo (red, credenciales, sin créditos, formato de respuesta) en `ExecutiveSummaryGenerationException`.
